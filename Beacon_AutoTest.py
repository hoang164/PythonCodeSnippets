...
def input_row_numbers():
    ...
    while True:
        try:
            ...
            print("Error: Input should be numeric numbers separated by a space.")
    return row_num_list


def extract_param_range_str_to_dict(model_name, sim_model_df):
    """
    ...
    :param model_name: string represents model name 
    ...
    """
    param_list = sim_model_df[sim_model_df["Sim_model"] == model_name]["Parameter_range_list"].values[0].split(",")
    ...
    for _i, _model in enumerate(model_list):
        ...
        if pd.isna(modified_params):
            ...
            for param_idx, param_item in enumerate(modified_params_name):
                value = modified_params_value[param_idx][0]  # delete last element in each sublist of modified params values
                ...
                if param_item not in params_range_value_dict:
                    ...
                    if float(minimum) > float(maximum) or float(minimum) < params_range_value_dict[param_item][0] or float(maximum) > params_range_value_dict[param_item][1] or float(step)+float(minimum) > float(maximum):
                        print("-----> Input %d: ERROR Row#%d  model %s parameter <%s> value is out of range. Simulation aborted\n"
                              % (_i+1, row_num, _model, param_item))
                        error_flag = 1
                    else:
                        find_finetune_var += 1
                    ...
                    for value_item in value_list:
                        if float(value_item) < params_range_value_dict[param_item][0] or float(value_item) > params_range_value_dict[param_item][1]:
                            print("-----> Input %d: ERROR Row#%d  model %s parameter <%s> value is out of range. Simulation aborted\n"
                                  % (_i+1, row_num, _model, param_item))
                            error_flag = 1
                    find_finetune_var += 1
                                                  
                elif float(value) < params_range_value_dict[param_item][0] or float(value) > params_range_value_dict[param_item][1]:
                    print("-----> Input %d: ERROR Row #%d  model %s parameter <%s> value is out of range. Simulation aborted\n"
                          % (_i+1, row_num, _model, param_item))
                    error_flag = 1
...
def convert_usecase_to_signal_list(usecase_dataframe, usecaselist):
    ...
    for _i in usecaselist:
        signal_id_list_with_libname = str(usecase_dataframe[usecase_dataframe["Row"] == int(float(_i))]["UseCase_set"].values[0])
...
        'MP_test'    param1: 2, param2: 3     5
    ...
    save_testinfo_eachrow_df = pd.DataFrame(df_col, index=range(0, 1))  # create a data frame
    ...
    for _i in params_dict[fine_tune]:
        ...
        for _param in params_dict:
            ...
            if _param != fine_tune:
                params_dict[_param] = params_dict[_param][0] if isinstance(params_dict[_param],list) else params_dict[_param]
                temp_parameters += ("," + _param[:-1] + ": " + str(params_dict[_param]))
            else:
                temp_parameters += ("," + _param[:-1] + ": " + str(_i))
            save_testinfo_eachrow_df.Model_Parameters = temp_parameters[1:] 
            save_testinfo_eachrow_df.Model_No = model                        
            save_testinfo_eachrow_df.SignalSet = None                      
            
        for _item in signal_no_list:
            save_testinfo_eachrow_df.SignalSet = _item
            test_info_dataframe = test_info_dataframe.append(save_testinfo_eachrow_df)
...
def create_simulink_input_df(test_dataframe, usecase_dataframe):
    ...
    for _i, _model in enumerate(model_list):
        ...
        modified_params_value = re.findall(r"([-]*[0-9]+[.]*[0-9]*:[-]*[0-9]+[.]*[0-9]*:[-]*[0-9]+[.]*[0-9]*"
                                           r"|[-]*[0-9]+[.]*[0-9]*(,[-]*[0-9]+[.]*[0-9]*)+|[-]*[0-9]+[.]*[0-9]*)", modifed_params) # require modify
...
def output_excel_create(test_info_dataframe, loglist_dataframe, log_filename):
    ...
    :return: none
    """
    #log_folder = "simulation_log"    # hardcoded , change needed
    #if not os.path.exists(log_folder):
    #    os.makedirs(log_folder)
        
    output_folder = "Input_excel_for_matalb_simulation" 
    ...
    output_files_dir_list = [os.path.abspath("%s/sim_%s.xlsx" % (output_folder, current_time))]*test_info_df_rownum
...
def matlab_simulation(entry):
    os.system('matlab -nodesktop -nosplash -r "Autotest_Beacon({0},{1},{2},{3},{4},{5},{6},{7},{8});exit"'.format(
...
def create_matlab_input_list(test_info_dataframe, datalib_dataframe, model_dataframe):
    ...
    for _i in range(num_sim_run):
        ...
        signal_path = (datalib_dataframe.iloc[signal_idx]["StudyName"] + "/" + datalib_dataframe.iloc[signal_idx]["US_machine"] + "/" + datalib_dataframe.iloc[signal_idx]["DataFolderName"])

        model_path = model_dataframe[model_dataframe["Sim_model"] ==
                                     test_info_dataframe.iloc[_i]["Model_No"]]["Sim_FilePath"].values[0]

        _entry0 = "'" + signal_name + ".mat"+"'"
        _entry1 = "'" + param+"'"
        _entry2 = "'" + str(_i) + "'"
        _entry3 = "'" + model + "'"
...
if __name__ == "__main__":
    """
    Main Code
    Assign names for input excel files 
    """
    MODEL_FILE = 'test_info_1.xlsx'   # model and its parameter
    DATA_FILE = 'RX_Data_Library.xlsx'  # data library and use case
    LOG_FILE = 'simulation_log.xlsx'  # simulation log file
    MP_NUM = 4

    print('-------Welcome to Beacon Automated Simulink Testing Bench------\n')
    print('....Model and its parameter information is stored in %s.\n' % MODEL_FILE)
    print('....Data library and Use case information is stored in %s.\n' % DATA_FILE)

    # loading model and library info from input excel files
    MODEL_DF = pd.read_excel(MODEL_FILE, sheet_name='Model')
    PARAMETER_DF = pd.read_excel(MODEL_FILE, sheet_name='Default_ParametersSet')[["Row", "Sim_model", "Parameters",
                                                                                  "Modified Parameters", "UseCases"]]
    USECASE_DF = pd.read_excel(DATA_FILE, sheet_name='TestUseCase')[["Row", "UseCase_set"]]
    DATALIB_DF = pd.read_excel(DATA_FILE, sheet_name='DataLibrary')
    LOGLIST_DF = pd.read_excel(LOG_FILE)

    # get user input row number and verify parameters and use cases
    input_rows_list = input_row_numbers()
    test_df = PARAMETER_DF.iloc[input_rows_list]

    find_error = 0
    find_error = param_verify(find_error, test_df, MODEL_DF)
    find_error = usecase_verify(find_error, test_df, USECASE_DF)

    if find_error != 0:
        exit()

    # Initiate the test
    test_info_df = create_simulink_input_df(test_df, USECASE_DF)
    matlab_entry = create_matlab_input_list(test_info_df, DATALIB_DF, MODEL_DF)
    output_excel_create(test_info_df, LOGLIST_DF, LOG_FILE)

    # Simulation test
    processors = mp.Pool(MP_NUM)
    processors.map(matlab_simulation, matlab_entry)
